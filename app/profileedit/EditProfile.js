    ...
    <div className="sm:hidden absolute top-full left-0 w-full bg-[#0F283C] text-white p-4">
      <nav>
        {tabs.map((tab) => (
          <button
            key={tab}
            className={`block p-2 text-left w-full ${
              selectedTab === tab ? 'text-white' : 'text-gray-300'
            }`}
            onClick={() => setSelectedTab(tab)}
          >
          ...
          <div className="md:w-2/3">
            <form className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  type="text"
                  className="mt-1 p-2 border border-[#9EB5C7] rounded w-full focus:outline-none focus:ring-2 focus:ring-blue-500 bg-[#F2F6FB]"
